...
def produceSongResult(artist, songName, explicit):
    songResult = ''
    ...
    if (oldSongName != songName):
        ...
        songResult = st.markdown(f'{i+1}: <a href="{songLink}" target="_blank" rel="noopener noreferrer"><b><i>{songName}</i></b></a>, by {artist}', unsafe_allow_html=True)
    ...
    try:
        ...
        with col6:
            for i in range(5):
                ...
                songName = results[i]
                produceSongResult(artist, songName, profanity_filter)
        with col7:
            for i in range(5, 10):
                artist = df_read[df_read.title == results[i]].artist.values[0]
                # Implementing the profanity filter over the song name
